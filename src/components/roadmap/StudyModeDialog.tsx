...
const StudyModeDialog = ({
  ...
  roadmapTitle,
  languageName,
  roadmapProgress = defaultProgress,
  ...
  const [promptLevel, setPromptLevel] = useState<"beginner" | "intermediate" | "advanced" | "custom">(
    getStoredPromptLevel,
  );
  ...
  const handleAIGenerate = async () => {
    ...
    try {
      ...
      const newStats: StudyStats = {
        ...
        lastStudyDate: today,
        streak:
          studyStats.lastStudyDate === new Date(Date.now() - 86400000).toDateString()
            ? studyStats.streak + 1
  ...
  const handleSaveExplanation = async () => {
    ...
    setIsSaving(true);
    try {
      const { error } = await supabase.from("saved_explanations").insert({
  ...
  const handleDeleteSaved = async (id: string) => {
    try {
      const { error } = await supabase.from("saved_explanations").delete().eq("id", id);
      ...
      <DialogContent className="max-w-3xl max-h-[85vh]">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="grid w-full grid-cols-6">
            ...
            <TabsTrigger value="result" disabled={!explanation && !isGenerating}>
              الشرح
            </TabsTrigger>
            <TabsTrigger value="quiz" disabled={quizQuestions.length === 0 && !isGeneratingQuiz}>
              اختبار
            </TabsTrigger>
          ...
          <TabsContent value="select" className="space-y-4">
            <div className="flex items-center justify-between">
              <p className="text-sm text-muted-foreground">اختر المواضيع التي تريد دراستها</p>
                    ...
                    <div className="mr-6 space-y-1">
                      {section.topics.map((topic) => (
                        <div key={topic.id} className="flex items-center gap-2">
                          ...
                          />
                          <span
                            className={`text-sm ${topic.title.startsWith("↳") ? "text-muted-foreground mr-2" : ""}`}
                          >
                            {topic.title}
                          </span>
                          {topic.completed && (
          ...
          <TabsContent value="result" className="space-y-4">
            <ScrollArea className="h-[350px] border rounded-lg p-4">
              ...
              {explanation && (
                <div className="prose prose-sm max-w-none dark:prose-invert whitespace-pre-wrap">{explanation}</div>
              )}
            </ScrollArea>
            ...
            {explanation && (
              <div className="flex gap-2">
                ...
                <Button variant="outline" onClick={handleSaveExplanation} disabled={isSaving}>
          ...
          <TabsContent value="quiz" className="space-y-4">
            ...
            {showResult && (
              <div className="text-center space-y-4 py-8">
                <div
                  className={`text-6xl mb-4 ${quizScore >= quizQuestions.length / 2 ? "text-green-500" : "text-orange-500"}`}
                >
              ...
              <div className="space-y-4">
                ...
                <div className="p-4 border rounded-lg bg-muted/30">
                  ...
                  {isCurrentAnswered && (
                    <div className="mt-4 p-3 bg-blue-500/10 border border-blue-500/30 rounded-lg">
                      <p className="text-sm">
                        <strong>التوضيح:</strong> {currentQuestion.explanation}
                      </p>
                    </div>
                  )}
                </div>
                ...
                <div className="flex justify-between">
                  {!isCurrentAnswered ? (
                    ...
                    <Button onClick={() => setShowResult(true)}>عرض النتيجة</Button>
                  )}
                </div>
              </div>
            )}
          ...
          <TabsContent value="settings" className="space-y-4">
            <div className="space-y-4">
                  ...
                  <div className="flex items-center space-x-2 space-x-reverse">
                    ...
                    <Label htmlFor="beginner" className="flex-1 cursor-pointer">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-muted-foreground">شرح مبسط جداً مع أمثلة سهلة</span>
                      </div>
                    </Label>
                  </div>
                  <div className="flex items-center space-x-2 space-x-reverse">
                    ...
                    <Label htmlFor="intermediate" className="flex-1 cursor-pointer">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-muted-foreground">شرح متوازن مع أمثلة واقعية</span>
                      </div>
                    </Label>
                  </div>
                  <div className="flex items-center space-x-2 space-x-reverse">
                    ...
                    <Label htmlFor="advanced" className="flex-1 cursor-pointer">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-muted-foreground">شرح تفصيلي متقدم مع تفاصيل تقنية</span>
                      </div>
                    </Label>
                  </div>
                  <div className="flex items-center space-x-2 space-x-reverse">
                    ...
                    <Label htmlFor="custom" className="flex-1 cursor-pointer">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-muted-foreground">تخصيص البرومبت بنفسك</span>
                      </div>
                    </Label>
                  </div>
                </RadioGroup>
              ...
              <div className="space-y-2">
                <Label className="text-sm text-muted-foreground">استخدم المتغيرات التالية في القالب:</Label>
